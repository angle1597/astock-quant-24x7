...
def calc_score_v87(klines, idx):
    ...
    return consec, score, chg5, chg10, vol_ratio, shrink, rsi6, int(shrink==1 and consec>=3)
...
def backtest_v87():
    ...
    for (code,) in stocks:
        ...
        for idx in range(30, len(klines) - 100):  # 持有100天
            buy_price = klines[idx][2]
            
            if buy_price >= 10 or buy_price <= 0:
                continue
            
            consec, score, chg5, chg10, vol_ratio, shrink, rsi6, has_shrink = calc_score_v87(klines, idx)
            
            # V87条件：评分>=75，且有缩量（保持质量）
            if consec < 3:
                continue
            if score < 75:
                continue
            if has_shrink != 1:  # 必须有缩量
                continue
    ...
    print("\n三策略对比:")
    print("-" * 70)
    print(f"{'指标':<12} {'V85':<15} {'V86':<15} {'V87':<15}")
    print("-" * 70)
    print(f"{'交易次数':<12} {'8':<15} {'9338':<15} {trade_count}")
    print(f"{'10%达标率':<12} {'87.5%':<15} {'32.5%':<15} {f'{r10_count/trade_count*100:.1f}%' if trade_count > 0 else 'N/A'}")
    print(f"{'30%达标率':<12} {'87.5%':<15} {'9.8%':<15} {f'{r30_count/trade_count*100:.1f}%' if trade_count > 0 else 'N/A'}")
    print(f"{'平均收益':<12} {'85.10%':<15} {'6.33%':<15} {f'{total_profit/trade_count:.2f}%' if trade_count > 0 else 'N/A'}")
    print(f"{'胜率':<12} {'100%':<15} {'56.2%':<15} {f'{win_count/trade_count*100:.1f}%' if trade_count > 0 else 'N/A'}")
    print("-" * 70)
