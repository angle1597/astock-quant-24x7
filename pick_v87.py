...
def pick_v87():
    ...
    for (code,) in stocks:
        ...
        consec, score, chg5, chg10, vol_ratio, shrink, rsi6 = calc_score_v87(klines)
        
        # V87条件：连涨>=3 + 评分>=75 + 必须有缩量
        if consec < 3: continue
        if score < 75: continue
        if shrink != 1: continue  # 必须有缩量
    ...
    print()
    print("V87条件: 连涨>=3 + 评分>=75 + 缩量")
    print("回测: 10%达标44.1%, 30%达标19.4%, 胜率67.6%")
    print(f"信号数: {len(candidates)}")
    print()
    
    for i, c in enumerate(candidates[:5]):
        print(f"{i+1}. {c['code']} | {c['price']:.2f}元 | 评分:{c['score']} | 连涨:{c['consec']}天")
        print(f"   5日:{c['chg5']:+.1f}% 10日:{c['chg10']:+.1f}% RSI:{c['rsi6']:.0f} 量比:{c['vol_ratio']:.2f}x")
